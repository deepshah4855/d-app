...
export class PatientEditComponent implements OnInit, OnDestroy {
	...
	addressSubject = new BehaviorSubject<string>('');
			...
			if (id && id > 0) {
				this.store.pipe(select(selectUserById(id))).subscribe(res => {
					if (res) {
						...
						this.addressSubject.next(this.user.streetAddress);
	...
	initUser() {
		...
		this.subheaderService.setBreadcrumbs([
			{ title: 'User Management', page: `user-management` },
			{ title: 'Users',  page: `user-management/users` },
			{ title: 'Edit user', page: `user-management/users/edit`, queryParams: { id: this.user.patientID } }
		]);
	}
	...
	createForm() {
		this.userForm = this.userFB.group({
			...
			fullname: [this.user.lastName, Validators.required],
  ...
  this.userForm.updateValueAndValidity();
	...
	prepareUser(): User {
		...
		_user.streetAddress = this.addressSubject.value;
		...
		_user.imageSource = this.user.imageSource;
		_user.patientID = this.user.patientID;
		_user.firstName = controls.username.value;
		_user.email = controls.email.value;
		_user.lastName = controls.fullname.value;
		// _user.occupation = controls.occupation.value;
		_user.mobilePhone = controls.phone.value;
