...
export class UsersListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['select', 'id', 'title', 'firstName', 'email', 'mobilePhone'];
	...
	ngOnInit() {
		...
		**/
		...
		const entitiesSubscription = this.dataSource.entitySubject.pipe(
			...
			// this.usersResult = res.slice(0,100);
			this.data = new MatTableDataSource(this.usersResult);
		});
		this.subscriptions.push(entitiesSubscription);

		// First Load
		of(undefined).pipe(take(1)).subscribe(() => { 
			this.loadUsersList();
		});
	}
	...
	deleteUser(_item: User) {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			// this.store.dispatch(new UserDeleted({ id: _item.patientID }));
